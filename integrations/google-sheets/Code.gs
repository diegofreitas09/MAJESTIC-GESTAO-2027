...
function doGet(e) {
  try {
    ...
    if (!p.action) return json_({ok:true,service:'MAJESTIC Google Sheets Database',version:DB_VERSION,at:new Date().toISOString()});
    authorize_(p.token);
    if (p.action === 'health') return json_(health_());
    if (p.action === 'list') return json_({ok:true,table:p.table,records:list_(p.table,p)});
    if (p.action === 'get') return json_({ok:true,table:p.table,record:get_(p.table,p.id)});
    if (p.action === 'changes') return json_({ok:true,table:p.table,records:changes_(p.table,p.since)});
...
function doPost(e) {
  try {
    ...
    const action = String(body.action || (body.events ? 'batch' : 'upsert')).toLowerCase();
    ...
    if (action === 'delete') return json_({ok:true,deleted:softDelete_(body.table,body.id,body.actor || 'app')});
...
function processBatch_(events,actor){return transaction_((events||[]).map(ev=>({action:String(ev.operation||ev.action||'upsert').toLowerCase(),table:ev.table,record:ev.record||{},id:ev.id||((ev.old_record||{}).id)})),actor);}
  ...
  try{
    (ops||[]).forEach((op,i)=>{try{
      ...
      if(op.action==='delete'){
        ...
        const rec=Object.assign({},op.record||{});if(!rec[idH]&&!rec.id)rec[idH]=Utilities.getUuid();
...
function normalize_(v){if(v===undefined||v===null)return '';if(v instanceof Date)return v;if(typeof v==='object')return JSON.stringify(v);return v;}
...
function audit_(acao,modulo,id,actor,data){try{const sh=SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('LOG_AUDITORIA');if(!sh)return;const h=headers_(sh),rec={id:Utilities.getUuid(),id_evento:Utilities.getUuid(),ocorrido_em:new Date().toISOString(),acao,modulo,entidade:modulo,registro_id:id,usuario_nome:actor,origem:'google-sheets-db',dados_novos:data,resultado:'OK',sync_status:'OK',sync_at:new Date().toISOString()};const row=h.map(k=>normalize_(rec[k]));sh.getRange(sh.getLastRow()+1,1,1,row.length).setValues([row]);}catch(_){}}
