...
export default function AtendimentoCRM({profile}){
 ...
 useEffect(()=>{carregar();carregarComercial();carregarAtendentes();const canal=supabase.channel('crm-equipe-sync-v7').on('postgres_changes',{event:'*',schema:'public',table:'gestao_clientes'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'gestao_atendimentos'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'produtos_comerciais'},carregarComercial).on('postgres_changes',{event:'*',schema:'public',table:'atendentes'},carregarAtendentes).subscribe();return()=>supabase.removeChannel(canal)},[]);
 ...
 function novo(){setSelecionado(null);setAtual(null);setAtendenteSelecionado('');setBusca('');setForm(vazio);setObs('');setItensSelecionados([]);setAcoes({visita:false,proposta:false,confirmou:false,matriculado:false,perdido:false})}
 function escolher(c){setSelecionado(c);setForm({...vazio,...c,data_nascimento:c.data_nascimento||'',proximo_contato_at:c.proximo_contato_at?String(c.proximo_contato_at).slice(0,16):''});setBusca('');const u=atendimentos.find(a=>a.cliente_id===c.id);setAtual(u?.status==='em_andamento'?u:null);setAtendenteSelecionado(u?.status==='em_andamento'&&u?.atendente_id?String(u.atendente_id):'');setItensSelecionados(Array.isArray(u?.orcamento_json)?u.orcamento_json.map(x=>x.id):[]);setAcoes({visita:['visita','proposta','decisao','matriculado'].includes(c.status_funil),proposta:['proposta','decisao','matriculado'].includes(c.status_funil),confirmou:['decisao','matriculado'].includes(c.status_funil),matriculado:!!c.matriculado,perdido:c.status_funil==='perdido'})}
 ...
 async function iniciar(e){
  ...
  if(r.error){setAviso(r.error.message);return}setSelecionado(cli);setAtual(r.data);setAviso(`Atendimento iniciado por ${atendente.nome}. A Direção já consegue acompanhar.`);await carregar()
 }
 ...
 return <section className="crmPage">
  ...
  {!atual&&<button className="primary crmStart" type="submit" disabled={!atendenteSelecionado}><PlayCircle size={18}/>{selecionado?'Retomar atendimento':'Iniciar e salvar atendimento'}</button>}</form>
