...
export default function AtendimentoCRM({profile}){
 ...
 useEffect(()=>{carregar();carregarComercial();carregarAtendentes();const canal=supabase.channel('crm-equipe-sync-v6').on('postgres_changes',{event:'*',schema:'public',table:'gestao_clientes'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'gestao_atendimentos'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'produtos_comerciais'},carregarComercial).on('postgres_changes',{event:'*',schema:'public',table:'atendentes'},carregarAtendentes).subscribe();return()=>supabase.removeChannel(canal)},[]);
 ...
 async function gerarPdfAtendimento(){
  ...
  const doc=new jsPDF({unit:'mm',format:'a4'}),logo=await carregarLogo(),atendente=atual?.atendente_nome||atendenteAtual?.nome||atual?.funcionario_nome||funcionario;
