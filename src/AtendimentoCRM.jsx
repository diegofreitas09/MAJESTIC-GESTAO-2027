...
export default function AtendimentoCRM({profile}){
 ...
 const [estadoSalvar,setEstadoSalvar]=useState('idle');
 ...
 useEffect(()=>{carregar();carregarComercial();carregarAtendentes();const canal=supabase.channel('crm-equipe-sync-v8').on('postgres_changes',{event:'*',schema:'public',table:'gestao_clientes'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'gestao_atendimentos'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'produtos_comerciais'},carregarComercial).on('postgres_changes',{event:'*',schema:'public',table:'atendentes'},carregarAtendentes).subscribe();return()=>supabase.removeChannel(canal)},[]);
 ...
 function escolher(c){setSelecionado(c);setForm({...vazio,...c,data_nascimento:c.data_nascimento||'',proximo_contato_at:c.proximo_contato_at?String(c.proximo_contato_at).slice(0,16):''});setBusca('');const u=atendimentos.find(a=>a.cliente_id===c.id);setAtual(u||null);setAtendenteSelecionado(u?.atendente_id?String(u.atendente_id):'');setItensSelecionados(Array.isArray(u?.orcamento_json)?u.orcamento_json.map(x=>x.id):[]);setEstadoSalvar('idle');setAcoes({visita:['visita','proposta','decisao','matriculado'].includes(c.status_funil),proposta:['proposta','decisao','matriculado'].includes(c.status_funil),confirmou:['decisao','matriculado'].includes(c.status_funil),matriculado:!!c.matriculado,perdido:c.status_funil==='perdido'})}
 ...
 async function salvar(){
  if(!selecionado||estadoSalvar==='saving')return;
  setEstadoSalvar('saving');setAviso('Enviando atualização para a Gestão...');
 ...
 async function gerarPdfAtendimento(){
  try{
   const atendente=atual?.atendente_nome||atendenteAtual?.nome||atual?.funcionario_nome||funcionario;
   await gerarPropostaComercialPDF({form,itens:itensOrcamento,atendente,observacao:obs});
   setAviso('Proposta comercial gerada para a família.');
  }catch(e){setAviso(e?.message||'Não foi possível gerar a proposta comercial.')}
 }
 ...
 async function relatorio(){
  ...
  doc.setFont('helvetica','normal');doc.setFontSize(10);doc.setTextColor(50,65,85);doc.text(`${dataInicio.split('-').reverse().join('/')} a ${dataFim.split('-').reverse().join('/')}`,logo?76:14,24);doc.text(`Gerado pelo acesso ${funcionario||'Gestão Majestic'} em ${new Date().toLocaleString('pt-BR')}`,logo?76:14,30);
 ...
 const atendentesDisponiveis=atendentes.filter(a=>a.ativo||String(a.id)===String(atendenteSelecionado));

 return <section className="crmPage">
  <div className="crmHero"><div><span>ATENDIMENTO MAJESTIC</span><h2>Atender bem, registrar sem burocracia.</h2><p>Selecione a atendente responsável, identifique a origem da família e salve todo o atendimento em um único fluxo.</p></div><button type="button" onClick={novo}><RotateCcw size={17}/>Novo atendimento</button></div>
  ...
  <form className="panel crmForm" onSubmit={iniciar}><div className="crmFormTitle"><div><h3>{selecionado?'Continuar atendimento':'Dados da família e do aluno'}</h3><p>O acesso identifica o sistema; a atendente selecionada identifica quem realizou o atendimento.</p></div><div className="crmAutoBadge">Etapa automática: <b>{LABELS[auto.etapa]}</b> • {auto.pct}%</div></div><div className="crmGrid">
   <label>Atendente responsável *<select required disabled={!!atual} value={atendenteSelecionado} onChange={e=>setAtendenteSelecionado(e.target.value)}><option value="">Selecione quem está atendendo</option>{atendentesDisponiveis.map(a=><option key={a.id} value={a.id}>{a.nome}{a.ativo?'':' • desativada'}</option>)}</select></label><label>Acesso do sistema<input readOnly value={funcionario||profile?.email||'Gestão Majestic'}/></label><label>Como conheceu? *<select required value={form.origem} onChange={e=>campo('origem',e.target.value)}><option value="">Selecione</option>{ORIGENS.map(x=><option key={x}>{x}</option>)}</select></label>
  ...
  <section className="panel crmReport"><div><CalendarDays size={22}/><div><h3>Atendimentos do dia ou período</h3><p>Relatório institucional com atendente responsável, indicadores, origem das famílias e detalhamento dos atendimentos.</p></div></div><label>Início<input type="date" value={dataInicio} onChange={e=>setDataInicio(e.target.value)}/></label><label>Fim<input type="date" value={dataFim} onChange={e=>setDataFim(e.target.value)}/></label><button type="button" className="primary" onClick={relatorio}><Download size={17}/>Baixar relatório</button></section>
